...
public class MutationSupporter {
    ...
    private static List<CtInvocationImpl> invocations;

    private static Set<CtMethod<?>> allMethods;

    private static Collection<CtType<?>> allClasses;
    ...
    public static List<CtInvocationImpl> getInvocations() {
        if (invocations == null)
            invocations = factory.getModel().getRootPackage().getElements(new TypeFilter<>(CtInvocationImpl.class));
        return invocations;
    }

    public static Collection<CtType<?>> getAllClasses(){
        if(allClasses == null)
            allClasses = factory.getModel().getAllTypes();
        return allClasses;
    }

    public static Set<CtMethod<?>> getAllMethods() {

        if (allMethods == null){
            Stream<Set<CtMethod<?>>> x = getAllClasses().stream().map(CtType::getMethods);
            allMethods = x.flatMap(Collection::stream).collect(Collectors.toSet());
        }
        return allMethods;
    }
